...
docker compose --env-file .env.docker -f docker/docker-compose.yml exec -T $SERVICE_NAME ls -la /home/node/.config/clawbr 2>/dev/null || echo "Clawbr config directory not found!"
docker compose --env-file .env.docker -f docker/docker-compose.yml exec -T $SERVICE_NAME cat /home/node/.config/clawbr/credentials.json 2>/dev/null || echo "credentials.json not found!"
